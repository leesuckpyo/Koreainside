...
Related MD, Change Log, Project Memory, and Decision Log updates are performed only when the user explicitly requests or approves them.
...
- 문서 통합, 삭제, 이름 변경, 이동, 새 문서 생성은 사용자가 명시 요청하거나 승인한 범위 안에서만 수행한다.
...
확인은 한 줄이면 충분하지만, 현재 사용자 명시 지시와 root `AGENTS.md` 기준의 별도 명시 승인으로 확인되어야 한다.
...
Implementation belongs to Codex within the approved scope."
