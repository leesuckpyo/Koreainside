...
실행 권한, scope, 보호, diff 검토, 승인 및 Git 규칙은 root `AGENTS.md`를 따른다.

이 문서는 승인된 업무의 유형별 참고 절차이며, 파일 변경 권한을 부여하지 않는다.

## 공통 원칙

- Project Charter를 우선 확인한다.
- Product Vision과 현재 저장소 상태를 확인한다.
- 기존 파일, 규칙, 기능, SEO, FAQ, HTML Text는 기본 보호한다. 사용자가 명시적으로 변경이나 삭제를 승인한 경우 해당 범위 안에서만 수행한다.
- 관련 문서 보정이 필요하면 대상과 이유를 보고하고, `AGENTS.md`에 따라 승인된 문서 작업만 수행한다.
- QA 수준과 범위는 `AGENTS.md`를 따른다. 저위험 단일 수정은 최소 QA를 적용하고, 페이지 전체, 공통구조, 대규모 변경은 영향에 맞는 강화 QA를 적용한다.
- 아래 참조 문서, Checklist, 완료 기준은 승인된 범위와 선택된 QA 수준에 해당하는 항목만 적용한다. 모든 작업에 전체 체크리스트를 강제하지 않는다.
- 일반 구현에서는 점수를 생성하거나 보고하지 않는다. 참조 문서의 점수제를 포함한 사업성·품질 점수 평가는 사용자가 명시 요청한 경우에만 수행한다.
...
6. 확정한 페이지 구조를 구현한다.
7. 관련 page spec 작성 또는 업데이트 필요성을 확인한다.
...
- SEO Standard를 통과한다.
- `review-checklist.md`의 해당 QA 항목을 충족한다.
...
3. 보존 대상과 승인된 변경·삭제 대상을 구분한다.
4. SEO, UX, 접근성 영향을 분석한다.
5. 정해진 개선 범위를 반영한다.
6. 관련 MD 업데이트 필요성을 확인한다.

### Checklist

- 승인되지 않은 섹션 삭제가 없는가?
- 승인된 변경 범위 밖의 SEO 메타데이터를 보존했는가?
- 승인된 변경 범위 밖의 FAQ를 유지했는가?
- 정보가 더 명확해졌는가?
- 모바일 가독성이 유지되는가?

### 완료 기준

- 승인된 변경·삭제 범위 밖의 기존 기능과 콘텐츠가 보존된다.
- 개선 목적이 충족된다.
...
4. 키워드 남용 여부를 점검한다.
5. 확정한 SEO 수정 사항을 반영한다.
6. SEO 변경 기록의 필요성을 확인한다.
...
5. 확정한 이미지를 대상 위치에 반영한다.
6. 이미지 출처와 사용 페이지 기록의 필요성을 확인한다.
...
5. 확정한 FAQ를 추가한다.
6. 관련 문서의 FAQ 방향 업데이트 필요성을 확인한다.
...
5. 확정한 기능을 구현한다.
6. 해당 QA 수준의 테스트를 수행하고 문서 업데이트 필요성을 확인한다.
...
- 기능이 요청 목적을 해결한다.
- 기존 기능이 회귀되지 않는다.
- Code Review Standard의 해당 QA 항목을 충족한다.
...
3. 최소 수정안을 작성한다.
4. 회귀 가능성을 점검한다.
5. 확인된 원인을 최소 범위로 수정한다.
6. 수정 내용과 남은 리스크를 보고하고 기록 필요성을 확인한다.
...
- Design System
- Page Template Standard
- `review-checklist.md`의 접근성 및 해당 QA 항목
- Anti-Pattern Standard
...
3. 선택된 QA 수준에 따라 필요한 기기 범위의 영향을 분석한다.
4. 과도한 애니메이션이나 장식 여부를 점검한다.
5. 확정한 디자인 개선 사항을 반영한다.
6. 디자인 변경 문서화의 필요성을 확인한다.
...
- `release-strategy.md`의 페이지 완료 기준
- `review-checklist.md`의 해당 릴리스 QA 항목
- SEO Standard
- Risk Management
...
6. `AGENTS.md`의 승인 경계에 따라 릴리스 준비 상태를 보고한다.

### Checklist

- `review-checklist.md`의 해당 릴리스 QA 항목을 충족했는가?
- Broken Link가 없는가?
- FAQ와 내부 링크가 정상인가?
- 이미지와 alt가 준비되었는가?
- 남은 리스크가 공개 가능한 수준인가?
...
1. 긴급성 여부를 판단한다.
2. 수정 범위를 최소화한다.
3. 긴급성을 이유로 `AGENTS.md`의 승인 절차를 생략하지 않는다.
4. 확정한 오류를 수정한다.
5. 사후 변경 기록의 필요성을 확인한다.
...
3. 기존 결정과 충돌하는지 점검한다.
4. 승인된 범위와 QA 수준에 맞춰 SEO, UX, 접근성, 성능, 유지보수성을 검토한다.
5. 확인된 문제와 남은 리스크를 보고한다.

### Checklist

- 요청 범위를 벗어나지 않았는가?
- 승인 없는 구조 변경이 없는가?
- 승인되지 않은 기존 기능 삭제가 없는가?
- 관련 문서 업데이트가 필요한지 확인했는가?
- 남은 리스크가 명확한가?
...
2. 기존 파일과 규칙을 기본 보호하되, 사용자가 명시 승인한 범위 안에서 수정·병합·삭제한다.
3. 관련 Standards와 충돌 여부를 점검한다.
4. 문서 간 정합성과 실제 수정 결과를 검토한다.
5. 변경 파일과 남은 리스크를 보고한다.

### Checklist

- 승인된 변경·삭제 범위 밖의 기존 문서를 보존했는가?
- 중복 또는 충돌이 없는가?
- 관련 상위 문서와 일치하는가?
- 변경 이유가 명확한가?
- 코드 파일을 수정하지 않았는가?
